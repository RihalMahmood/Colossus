...
export default function FileCard({ file, onDeleted }) {
  ...
  return (
    <div className={`${isDark ? "file-card-dark" : "file-card-light"} group`}>
      {/*File type icon*/}
      <div className="flex items-start justify-between mb-3">
        <div className="text-3xl select-none">{typeInfo.icon}</div>
        {file.isChunked && (
          <div className={
            `flex items-center gap-1 px-2 py-0.5 rounded-full text-xs ${isDark ?
              "bg-violet-500/20 border border-violet-500/30 text-violet-300" :
              "bg-[#f0ebfe] border border-[#e9e0fd] text-[#8b5cf6]"
            }`}>
            <Layers size={10} />
      ...
      {/*File name*/}
      <p
        className={`font-body font-medium text-sm truncate mb-1 ${isDark ? "text-white" : "text-gray-900"}`}
        title={file.name}>
        {file.name}
      </p>
      ...
      <div className="space-y-1 mb-4">
        <div className="flex items-center justify-between text-xs text-white/30 font-mono">
          <span className={isDark ? "text-white/30" : "text-gray-400"}>{formatBytes(file.totalSize)}</span>
          <span className={typeInfo.color}>{typeInfo.label}</span>
        </div>
        <p className={`text-xs font-body ${isDark ? "text-white/20" : "text-gray-300"}`}>
      ...
      {!file.isChunked && file.singleDriveAccountEmail && (
        <div className={
          `flex items-center gap-1.5 text-xs font-body mb-3 truncate ${isDark ? "text-white/20" : "text-gray-300"
          }`}>
          <HardDrive size={11} />
      ...
      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
        <button
          ...
          className={`btn btn-xs flex-1 gap-1 ${isDark ?
            "bg-purple-600/20 border border-purple-500/30 text-purple-300 hover:bg-purple-600/30" :
            "bg-[#f0ebfe] border border-[#e9e0fd] text-[#8b5cf6] hover:bg-[#e9e0fd]"
            }`}
        >
          <Download size={12} />
          Download
        ...
        <button
          ...
          className={`btn btn-xs btn-ghost ${isDark ?
